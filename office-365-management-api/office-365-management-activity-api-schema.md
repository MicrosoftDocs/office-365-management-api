...
|UserId|Edm.string|Yes|The UPN (User Principal Name) of the user who performed the action (specified in the Operation property) that resulted in the record being logged; for example, `my_name@my_domain_name`. Note that records for activity performed by system accounts (such as app@sharepoint, SHAREPOINT\system, or NT AUTHORITY\SYSTEM) are also included. System accounts often perform activities that require elevated permissions. Audit records that indicate a system account performed the action are often triggered by user and admin activity. In these situations, the system account is performing the activity on behalf of the user or admin who initiated the action. For specific information about the app@sharepoint system account for audit records related to SharePoint activity, see [The app@sharepoint system account](https://docs.microsoft.com/microsoft-365/compliance/search-the-audit-log-in-security-and-compliance#the-appsharepoint-system-account).|
