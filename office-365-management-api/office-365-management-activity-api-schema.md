...
The UserId and UserKey of these events are always SecurityComplianceAlerts. There are three types of alert signals that are stored as the value of the Operation property of the common schema:
...
- AlertUpdated - An update was made to the alert event metadata. This alert is logged when the status of the alert is changed (for example, from "Active" to "Resolved") and when someone adds a comment to the alert.
